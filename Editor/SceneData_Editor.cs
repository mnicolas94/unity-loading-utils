...
namespace ZeShmouttsAssets.DataContainers.EditorScripts
{
	...
	public class SceneData_Editor : Editor
	{
		...
		private const string MenuItemPath = "Assets/Create/Facticus/LoadingUtils/Scene Data from SceneAsset";

		[MenuItem(MenuItemPath, false, 10)]
		private static void CreateFromSceneObject()
		{
			var scenes = Selection.GetFiltered<SceneAsset>(SelectionMode.Assets);
			foreach (var sceneAsset in scenes)
			{
				CreateSceneData(sceneAsset);
			}
		}
